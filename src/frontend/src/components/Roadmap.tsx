            ...
            <div className="flex justify-between items-center">
              {roadmapPhases.map((phase, index) => (
                <div key={index} className="relative flex flex-col items-center">
                  {/* Timeline dot */}
                  <div className={`w-4 h-4 rounded-full border-4 ${getStatusColor(phase.status)} mb-4`} />
                  
                  {/* Phase card */}
                  <div className={`w-64 p-6 rounded-xl border backdrop-blur-sm ${getStatusColor(phase.status)} transform hover:scale-105 transition-all duration-300`}>
          ...
          {roadmapPhases.map((phase, index) => (
            <div key={index} className="relative">
              ...
              <div className="flex items-start">
                {/* Timeline dot */}
                <div className={`w-12 h-12 rounded-full border-4 ${getStatusColor(phase.status)} flex items-center justify-center mr-4 flex-shrink-0`}>
                  {getStatusIcon(phase.status)}
                </div>
                
                {/* Phase content */}
                <div className={`flex-1 p-6 rounded-xl border backdrop-blur-sm ${getStatusColor(phase.status)}`}>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-sm font-semibold text-red-500">{phase.phase}</span>
