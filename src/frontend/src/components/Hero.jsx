        ...
        <div className="absolute top-16 right-0 bg-black/90 backdrop-blur-sm rounded-xl p-4 border border-red-900 shadow-red-glow min-w-48">
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-gray-300 text-sm font-medium">{speed.toFixed(1)}x</span>
              <button
...
const PortalAnimation = ({ speed }) => {
  const meshRef = useRef(null);
  const portalRingsRef = useRef(null);
  const energyParticlesRef = useRef(null);
  const pentagramsRef = useRef(null);
    ...
    if (portalRingsRef.current) {
      portalRingsRef.current.children.forEach((ring, index) => {
        ...
        if (ringMesh.material instanceof THREE.MeshStandardMaterial) {
          ...
          ringMesh.material.opacity =
            0.7 + Math.sin(time * baseOpacitySpeed * speed + offset) * 0.2;
        }
      });
    }
    ...
    if (pentagramsRef.current) {
      pentagramsRef.current.children.forEach((pentagram, index) => {
        ...
        if (pentagramMesh.material instanceof THREE.MeshStandardMaterial) {
          ...
          pentagramMesh.material.opacity =
            0.3 + Math.sin(time * baseOpacitySpeed * speed + offset) * 0.2;
        }
      });
    }
  });
    ...
    portalRings.push(
      <group key={i} position={[0, 0, z]}>
        ...
        {Array.from({ length: 8 }).map((_, beamIndex) => {
          ...
          return (
            <mesh key={beamIndex} position={[x, y, 0]} rotation={[0, 0, angle]}>
  ...
  for (let i = 0; i < 8; i++) {
    ...
    pentagrams.push(
      <mesh key={i} position={[x, y, z]} scale={[scale, scale, 1]} geometry={pentagramGeometry}>
        <meshStandardMaterial
          color={color}
          emissive={emissive}
  ...
  return (
    <group ref={meshRef}>
      ...
      <group ref={pentagramsRef}>{pentagrams}</group>

      {/* Portal rings - immediately visible */}
      <group ref={portalRingsRef}>{portalRings}</group>

      {/* Energy particles - immediately visible */}
      <group ref={energyParticlesRef}>{energyParticles}</group>
      ...
      {Array.from({ length: 6 }).map((_, i) => {
        ...
        return (
          <mesh key={i} position={[x, y, -105]} rotation={[0, 0, angle]}>
            <cylinderGeometry args={[0.06, 0.06, 180, 8]} />
            <meshBasicMaterial color="#ff8888" opacity={0.4} transparent />
          </mesh>
        );
      })}
      ...
      {Array.from({ length: 24 }).map((_, i) => {
        ...
        return (
          <mesh key={i} position={[0, 0, z]} scale={[scale, scale, 1]}>
            ...
            <meshBasicMaterial color="#ff2222" opacity={0.25 - i * 0.01} transparent />
          </mesh>
        );
      })}
